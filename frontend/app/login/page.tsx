...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 🟢 2️⃣ LOGIN (Email + Password) - Use Supabase
      ...
      if (result.user) {
        ...
        const redirectPath = authAPI.getRoleBasedRoute(result.user.role);
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (errorMessage.includes('Popup blocked')) {
        setErrors(prev => ({ ...prev, password: 'Please allow popups and try again, or use email/password login.' }));
      } else {
        setErrors(prev => ({ ...prev, password: errorMessage }));
      }
      setIsLoading(false);
  ...
  return (
    <main className="min-h-screen bg-white flex items-center justify-center px-4 animate-in fade-in duration-500">
